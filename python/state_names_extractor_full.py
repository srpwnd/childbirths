...
sq_cursor.execute("SELECT Id, Name, Year, Gender, State, Count FROM StateNames;")
...
pg_cursor.execute("TRUNCATE TABLE dbt_raw_data.state_names;")
...
for batch in iter(lambda: sq_cursor.fetchmany(batch_size), []):
    ...
    pg_cursor.execute(f"INSERT INTO dbt_raw_data.state_names(id, name, year, sex, state, count) VALUES {values};")
