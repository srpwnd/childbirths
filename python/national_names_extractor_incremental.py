...
pg_cursor.execute("SELECT COALESCE(MAX(year), 1700) FROM dbt_raw_data.national_names;")
result = pg_cursor.fetchone()

sq_cursor.execute(f"SELECT Id, Name, Year, Gender, Count FROM NationalNames WHERE year > {result[0]};")
...
for batch in iter(lambda: sq_cursor.fetchmany(batch_size), []):
    ...
    pg_cursor.execute(f"INSERT INTO dbt_raw_data.national_names(id, name, year, sex, count) VALUES {values} ON CONFLICT (id) DO UPDATE SET count = EXCLUDED.count;")
